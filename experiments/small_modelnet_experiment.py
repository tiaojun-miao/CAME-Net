...
from training.data_utils import collate_fn
from training.train import load_checkpoint, train_came_net
...
def create_experiment_artifacts(
    *,
    ...
    if extra_artifacts:
        _write_json(artifact_dir / "extra_artifacts.json", extra_artifacts)
...
def resolve_experiment_class_names(
    *,
    ...
    if class_protocol == "small5":
        return tuple(DEFAULT_SMALL_EXPERIMENT_CLASSES)
    if class_protocol != "full40":
        raise ValueError(f"Unsupported class_protocol: {class_protocol}")

    root = resolved_data_root or resolve_modelnet_root(data_root)
    ...
    return tuple(dataset.class_names)
    ...
    *,
    ...
    allowed_classes = list(
        resolve_experiment_class_names(
            class_protocol=getattr(config, "class_protocol", DEFAULT_CLASS_PROTOCOL),
