...
from .scannet_point_relevance_figure import (
    ScanNetPointRelevanceFigureConfig,
    generate_scannet_point_relevance_figure,
)


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a point-wise relevance heatmap figure on ScanNet scenes.")
    ...
    parser.add_argument("--scene-ids", nargs="+", required=True, help="One or more ScanNet scene ids")
    parser.add_argument("--queries", nargs="+", required=True, help="One target query label per scene row")
    parser.add_argument("--output", required=True, help="Output path stem, e.g. artifacts/figures/point_relevance_scannet")
    ...
    result = generate_scannet_point_relevance_figure(
        ScanNetPointRelevanceFigureConfig(
